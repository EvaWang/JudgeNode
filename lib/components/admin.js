...
module.exports = {
	...
	load_problem_content: function(pid, callback) {
		...
		var cmd = 'SELECT problems.*, DATE_FORMAT(deadline, "%Y-%m-%dT%H:%i") AS deadline FROM problems left join problem_condition on problems.pid = problem_condition.pid where problems.pid = ?';
		var pconfig = {};
	...
	update_problem_condition:function(pconfig){
		return new Promise(function(resolve, reject) {
			var cmd = 'INSERT INTO problem_condition (pid, deadline) VALUES (?, ?) ON DUPLICATE KEY UPDATE deadline=?;';
			connection.query(cmd, [pconfig.pid, pconfig.deadline, pconfig.deadline], function(err, result) {
	...
	load_accountlist: function(callback) {
		var cmd = 'SELECT * FROM users WHERE class IS NOT NULL ORDER BY lgn';
		connection.query(cmd, [], function(err, result) {
			callback(result);
		});
	},
	...
	create_problem_content: function(pconfig, callback) {
		...
		var cmd = 'INSERT INTO problems SET pub = ?, ttl = ?, level = ?, porder = ?, pid = ?, ts = ?, src= ? ';
		connection.query(cmd, [pconfig.pub, pconfig.ttl, pconfig.level, pconfig.porder, pid, new Date().getTime(), 'src'], function(err, result) {
			console.log(err);
			if (err)
				return callback();
			save_file();
